...
The flow contains a few basic components an OI4 complient OEC layer might exist of:
...
3. An additional Device, which gets covered from the _OTConnector_. This device is represented througt the application - therefore it has no MQTT client and communicated through the application.
...
## Pitfalls 
1. To use this flow in a larger context, e.g. to simulate onboarding or usage of AAS, please be sure to change at least the serial numbers of each asset (Edge Device, Appliaction and Device) to generate different instances. To do so, just edit the _mam object_ inside the function nodes called `init`. 
